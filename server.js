...
io.on("connection", function (socket) {
  ...
  socket.on("join", function (room) {
    logIt("В комнату вошел клиент", room);
    ...
    if (numClients === 0) {
      ...
      logIt("Трансляция готового сообщения", room);
  ...
  socket.on("token", function (room) {
    logIt("Полученный запрос токена", room);
    twilio.tokens.create(function (err, response) {
      if (err) {
        logIt(err, room);
      } else {
        logIt("Сгенерирован токен. Возврат в клиент браузера", room);
  ...
  socket.on("candidate", function (candidate, room) {
    logIt("Принят кандидат. Трансляция...", room);
    socket.broadcast.to(room).emit("candidate", candidate);
  });

  // Relay offers
  socket.on("offer", function (offer, room) {
    logIt("Полученно предложение. Трансляция...", room);
    socket.broadcast.to(room).emit("offer", offer);
  });

  // Relay answers
  socket.on("answer", function (answer, room) {
    logIt("Получены ответы. Трансляция...", room);
