...
var VideoChat = {
  ...
  requestMediaStream: function (event) {
    ...
    navigator.mediaDevices
      ...
      .catch((error) => {
        logIt(error);
        logIt(
          "Не удалось получить локальное видео с веб-камеры, проверьте настройки конфиденциальности веб-камеры"
        );
  ...
  onMediaStream: function (stream) {
    ...
    Snackbar.show({
      text: "Вот ссылка на соединение для вашего звонка: " + url,
  ...
  onCandidate: function (candidate) {
    // Update caption text
    captionText.text("Поиск другого пользователя... соединение");
...
function chatRoomFull() {
  alert(
    "Чат-комната полна. Проверьте, что у вас нет нескольких открытых вкладок, или попробуйте использовать новую ссылку на комнату"
  );
...
function pauseVideo() {
  ...
  if (videoIsPaused) {
    localVideoText.text("Видео на паузе");
    localVideoText.show();
...
function swap() {
  ...
  if (!VideoChat.connected) {
    alert("Вы должны присоединиться к вызову, прежде чем сможете поделиться своим экраном.");
    return;
  }
...
function requestToggleCaptions() {
  ...
  if (!VideoChat.connected) {
    alert("Вы должны быть подключены к одноранговому узлу, чтобы использовать субтитры");
    return;
  }
  if (receivingCaptions) {
    ...
    Snackbar.show({
      text:
        "Это экспериментальная особенность. Живая подпись требует, чтобы другой пользователь использовал Chrome",
      width: "400px",
      pos: "bottom-center",
      actionTextColor: "#616161",
      duration: 10000,
...
function recieveCaptions(captions) {
  ...
  if (captions === "notusingchrome") {
    alert(
      "Другой вызывающий абонент должен использовать chrome для работы этой функции. Видео с субтитрами отключены."
    );
    receivingCaptions = false;
    captionText.text("").fadeOut();
    captionButtontext.text("Start Live Caption");
  ...
  if (
    ...
    if (document.pictureInPictureElement) {
      ...
      remoteVideoVanilla.requestPictureInPicture().catch((error) => {
        alert(
          "Вы должны быть соединены с другим человеком, чтобы войти в картину в картине."
        );
      });
    }
  } else {
    alert(
      "Картинка в картинке не поддерживается в вашем браузере. Рассмотрите возможность использования Chrome или Safari."
    );
  }
}
//Picture in picture
...
function startUp() {
  ...
  Snackbar.show({
    text: "Пожалуйста разрешите доступ к микрофону и веб-камере",
    actionText: "Показать мне как",
    width: "455px",
    pos: "top-right",
    actionTextColor: "#616161",
    duration: 50000,
  ...
  captionText.text("Ожидание присоединения другого пользователя...").fadeIn();
